...
(() => {
    ...
    const DETECTION_RULES = [
        ...
        { name: 'Socket / Raw Network',       severity: 'medium',   pattern: /(?:socket\.socket|SOCK_STREAM|SOCK_DGRAM|WSAStartup|connect\(|bind\(|listen\(|accept\()/gi, desc: 'Raw socket operations — could be C2 communication' },
        ...
        { name: 'Password / Credential Keywords', severity: 'high', pattern: /(?:password|passwd|credential|login.*data|keychain|vault)/gi, desc: 'References passwords or credentials' },
        ...
        { name: 'Minecraft Mod Framework',    severity: 'medium',   pattern: /(?:net\.fabricmc|net\.minecraftforge|fml|mixin)/gi, desc: 'References Minecraft modding APIs' },
        { name: 'WeedHack / Known RAT',       severity: 'critical', pattern: /(?:WeedHack|weedhack|SessionStealer|Rat|TokenGrabber|DiscordGrabber)/gi, desc: 'Known Minecraft RAT signature' }
    ];
        ...
        if (isJar && window.JSZip) {
            try {
                ...
                for (const [path, zipEntry] of Object.entries(zip.files)) {
                    ...
                    if (pathLower.includes('weedhack') || pathLower.includes('stealer') || pathLower.includes('discord') || pathLower.includes('webhook') || pathLower.includes('grabber')) {
                        suspicious = true;
                        scoreBonus += 15;
                    }
